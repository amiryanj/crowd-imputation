...
import torchvision
import torch
from AggNet import SetNet, loss_fn
import numpy as np
import yaml
import argparse
...
if __name__ == '__main__':
    ...
    if exp_name == 'lfw':
        ...
        dataset_train = VGG_Faces2(training_dataset_root, split='train', upper=110)
        validation_dataset_root = '/nfs/nas4/marzieh/marzieh/VGG_Face2/train/'
        dataset_validation = VGG_Faces2(validation_dataset_root, split='validation', upper=110)
    #  --------------------------------------------------------------------------------------
    ...
    if start:
        ...
        with torch.no_grad():
            ...
            for batch_idx, (data, target, img_file, class_id) in enumerate(train_loader):
                ...
                f_set = model(data_set, m=m_set)  # single vector per set
                f_v = model(data_query, m=1)  # single vector per query
                loss_outputs = loss_fn(f_set, f_v, m_set)
                tot_loss += loss_outputs

                H0_batch = next(iter(H0_loader))
                Ptp01[batch_idx], Ptp05[batch_idx] = model.acc_authentication(H0_batch, target, n_classes, f_set, f_v, m_set)
